...
export type SaveV3={
 ...
 audio:AudioSave;
 pieces:{activePieceId:string|null;activePieceSequence:number|null;nextPieceSequence:number;textureVersion:'math-piece-v1';activePose:PiecePoseSave|null;pendingSpawnAt:number|null};
 opening:{hasCompletedInitialOpening:boolean};
};
export type Save=SaveV1|SaveV2|SaveV3;
...
export async function save(v:SaveV3){try{const d=await db();await new Promise<void>((resolve,reject)=>{const tx=d.transaction(STORE,'readwrite');tx.objectStore(STORE).put(v,KEY);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error)})}catch{/* Play remains available if storage is blocked. */}}
